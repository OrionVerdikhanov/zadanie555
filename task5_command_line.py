...
def collect_info(directory_path):
    """
    Собирает информацию о содержимом директории и сохраняет в лог-файл
    ...
    for entry in os.listdir(directory_path):
        ...
        # Запись в лог
        extension_for_log = file_info.extension if file_info.extension else "N/A"
        entry_type = "Directory" if file_info.is_directory else "File"
        logging.info(
            f"{file_info.name} | {extension_for_log} | {entry_type} | {file_info.parent_directory}"
        )

def main():
    """Основная функция для обработки командной строки и сбора информации."""
    parser = ArgumentParser(
        description="Сбор информации о содержимом директории и запись в лог."
    )
    ...
    try:
        ...
        print(f'Информация о содержимом директории "{directory_path}" успешно записана '
              f'в файл "directory_contents.log".')
